...
// FilterDatabases returns a copy of the ShardInfo, with shards associated
// with the given databases present.
func (s ShardInfos) FilterDatabases(filter []string) ShardInfos {
	var a ShardInfos
	...
	for _, si := range s {
		if slices.Exists(filter, si.Database) {
			a = append(a, si)
		}
	}
	return a
